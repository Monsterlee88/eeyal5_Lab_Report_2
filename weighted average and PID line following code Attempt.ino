...
float green = analogRead(GREEN);
float blue = analogRead(BLUE);
float purple = analogRead(PURPLE);
float yellow = analogRead(YELLOW);
float pink = analogRead(PINK);
float white = analogRead(WHITE);
...
{
const float weights[] = {-33, -20, -6, 6, -20, -33};
float Num = (weights[0] * green + weights[1] * blue + weights[2] * purple + weights[3] * yellow + weights[4] * pink + weights[5] * white);
float Den = (green + blue + purple + yellow + pink + white);
float Error = (Num/Den);
